...
function add_tape_old($db, $tape_number, $label, $type, $capacity, $container, $backup_set, $carton) {
    ...
    function get_container_by_id($db, $id) {
...
function get_all_from_type($db, $type_id, $active=1) {
...
*/
    ...
    function write_backupset_report($db, $backupset_id, $filename) {
        ...
        try {
            ...
            $excel->writeLine($header);
            $excel->writeLine($start_line);
            $excel->writeLine($end_line);
    ...
    function write_full_report($db, $filename) {
        try {
        $excel= new ExcelWriter("excel/".$filename);
        ...
        foreach($backupsets as $backupset_info) {
            ...
            $end_line = array("<B>End Date</B>", $backupset_end);
    ...
    function write_all_containers_report($db, $filename) {
        try {
        $excel= new ExcelWriter("excel/".$filename);
        ...
        foreach($containers as $container) {
            ...
            $parent_line = array("Located in:".$container_location);
            
            $excel->writeLine($header);
            $excel->writeLine($type_line);
            $excel->writeLine($parent_line);
